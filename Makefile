...
install:
	...
	docker exec hide-server /bin/sh -c "sudo sed -i \"/^# IPv4 local connections:/a \host hide hide 0.0.0.0/0 md5\" /etc/postgresql/14/main/pg_hba.conf"
	docker exec hide-server /bin/sh -c "sudo sed -i \"/^# IPv4 local connections:/a \host hide hide 127.0.0.1/32 trust\" /etc/postgresql/14/main/pg_hba.conf"
